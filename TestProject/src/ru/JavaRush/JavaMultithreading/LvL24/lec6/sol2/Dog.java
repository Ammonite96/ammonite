    ...
    public Sayable toSayable(final int i) {
        class DogPet extends SuperDog implements Sayable {
            private String getName() {
                String nameDog = String.format("%s%s%s", getSuperQuotes(), name, getSuperQuotes());
                return nameDog;
            }
            ...
            public String say() {
                ...
                if (i >= 1) {
                    ...
                    char[] charsA = new char[i];
                    for (int j = 0; j < charsA.length; j++) {
                        charsA[j] = 'а';
                    }
                    System.out.println(i);
                    StringBuilder builder = new StringBuilder("гав");
                    builder.delete(1, 2).insert(1, charsA);
                    return String.format("%s лает %s! %s", getName(), builder.toString(), formatter.format(currentDate));
                }
                return "";
            }
        }
